...
export function NewTaskModal({
  ...
  const save = async () => {
    if (!canSubmit || saveState === "saving" || saveState === "saved") return;
    setSaveState("saving");
    setSaveError(null);
    ...
    if (draft.status === "done") {
      body.doneAt = todayDateInputValue();
    }
    try {
      await onCreate(body);
      setSaveState("saved");
      ...
      setSaveState("error");
      setSaveError(formatApiError(e, "作成"));
    }
  };
      ...
      >
        ...
        >
          {errored && saveError ? (
            ...
            <>
              <span style={{ fontFamily: "SF Mono, monospace" }}>⌘↵</span> で作成
              ·{" "}
        ...
        <button
          ...
          style={{
            ...
            color: "#fff",
            boxShadow: canSubmit
              ? "0 2px 12px rgba(53,37,205,.22)"
              : "none",
            opacity: (!canSubmit || saving) && !saved ? 0.55 : 1,
          }}
        >
          {saving && (
            <svg
